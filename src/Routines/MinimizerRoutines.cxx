...
MinimizerRoutines::MinimizerRoutines(int argc, char* argv[]) {
  ...
  LOG(MIN) << "[ NUISANCE ]: Setting VERBOSITY=" << verbocount << std::endl;
  LOG(MIN) << "[ NUISANCE ]: Setting ERROR=" << errorcount << std::endl;
...
void MinimizerRoutines::SetupMinimizerFromXML() {
  ...
  for (size_t i = 0; i < parkeys.size(); i++) {
    ...
    if (parstate.find("ABS") != std::string::npos) {
      ...
      LOG(MIN) << "ParStep: " << parstep << " (" << oparstep << ")."
                << std::endl;
...
void MinimizerRoutines::GetMinimizerState() {
  ...
  if (fMinimizer->CovMatrixStatus() > 0) {
    // Fill Full Covar
    LOG(MIN) << "Filling covariance" << std::endl;
...
void MinimizerRoutines::SaveMinimizerState() {
  ...
  LOG(MIN) << "Saving Minimizer State" << std::endl;
  if (!fMinimizer) {
...
void MinimizerRoutines::SetupCovariance() {
  ...
  if (fMinimizer) {
    LOG(MIN) << "NFREE FROM MINIMIZER" << std::endl;
    NFREE = fMinimizer->NFree();
    NDIM = fMinimizer->NDim();
  } else {
    NDIM = fParams.size();
    for (UInt_t i = 0; i < fParams.size(); i++) {
      LOG(MIN) << "Getting Param " << fParams[i] << std::endl;
  ...
  int countfree = 0;
  for (UInt_t i = 0; i < fParams.size(); i++) {
    LOG(MIN) << "Getting Param " << i << std::endl;
    LOG(MIN) << "ParamI = " << fParams[i] << std::endl;
  ...
  LOG(MIN) << "Filling Matrices" << std::endl;
  ...
  LOG(MIN) << "   Set the covariance" << std::endl;
  return;
};
