...
GENIEWeightEngine::GENIEWeightEngine(std::string name) {
  ...
  // Setup the GENIE ReWeighting engine
  fCalcName = name;
...
void GENIEWeightEngine::IncludeDial(std::string name, double startval) {
  ...
  if (ccqetype != "kModeZExp" && (name == "ZExpA1CCQE" || name == "ZExpA2CCQE" || name == "ZExpA3CCQE" || name == "ZExpA4CCQE")) {
    ERR(WRN) << "Found a Z-expansion parameter in GENIE although the GENIE ReWeighting engine is set to use Llewelyn-Smith and MaQE!" << std::endl;
    ERR(WRN) << "Forcing GENIE ReWeight CCQE to enable z-expansion" << std::endl;
    rwccqe->SetMode(GReWeightNuXSecCCQE::kModeZExp);
  }

  if ((ccqetype != "kModeMa" && ccqetype != "kModeMaNormAndMaShape") && (name == "MaCCQE")) {
    ERR(WRN) << "Found MaCCQE parameter in GENIE although the GENIE ReWeighting engine is set to not use this!" << std::endl;
